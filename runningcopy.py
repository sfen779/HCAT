...
from lib.test.evaluation import Sequencecopy, Trackercopy
import torch


def _save_tracker_output(seq: Sequencecopy, tracker: Trackercopy, output: dict):
    ...
    def save_conf(file,data):
        conf_scores = np.array(data).astype(float)
        np.savetxt(file,conf_scores,delimiter='\t', fmt = '%f')
    ...
    for key, data in output.items():
        ...
        if key =="conf_score":
            print("conf_score saved")
            if isinstance(data[0],(dict, OrderedDict)):
                data_dict = _convert_dict(data)

                for obj_id, d in data_dict.items():
                    score_file = "{}_{}_conf_score.txt".format(base_results_path, obj_id)
                    save_conf(score_file,d)
            else:
                #singel-obj mode
                score_file = '{}_conf_score.txt'.format(base_results_path)
                save_conf(score_file, data)
...
def run_sequence(seq: Sequencecopy, tracker: Trackercopy, debug=False, num_gpu=8):
